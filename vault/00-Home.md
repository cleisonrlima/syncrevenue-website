...
- **Epic 6 (Visual Design Refresh) in-progress 2026-05-19** — Stories 6.3, 6.4, 6.5, and 6.6 are `done` after review; 6.6 received a pill-label fix so ClientReferences status chips now use the exact story labels while preserving the agency allowlist. Latest: Story 6.13 closed the Story 6.12 deferred work: legacy i18n stragglers migrated (`references.cta`, `forms.demo.*`, `forms.contact.*` subtrees deleted; consumers moved to surviving namespaces; audit GDS dropdown trimmed to canonical 4-value list with legacy values preserved on the wire); Lighthouse `heading-order` audit fixed via sr-only `<h2>` bridge in Hero; color-contrast violations on LanguageSwitcher buttons + Privacy email link fixed (a11y now 1.00 on `/` and `/privacy` both form factors); desktop CLS = 0 (was 0.184) via self-hosted Plus Jakarta Sans variable woff2 with preload — Google Fonts CDN removed; mobile LCP improved from 3,913–4,060 ms to 2,884–2,942 ms via airplane re-encode (webp variants 4–11 KB) + `<picture>` LCP candidate + image preload. Most LHCI thresholds restored to pre-6.12 values; mobile `largest-contentful-paint` set to 3,100 ms (was target 2,500 ms) with rationale — closing the residual ~400 ms gap requires SSG/prerender, deferred to Epic 5. Post-fix LHCI report: `_bmad-output/implementation-artifacts/epic-6-lhci-report-2026-05-19-post-fix/README.md`. Sober single-accent palette `#3D6FE0`; UX-DR2/DR3 deliberately superseded; all existing functional surfaces (form validation, locale parity, ClientReferences allowlist) preserved. Source: `_bmad-output/design-handoffs/syncsirius-website-2026-05-17/`.
