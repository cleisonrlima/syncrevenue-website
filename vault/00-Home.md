...
- **Epic 7 in-progress:** Stories 7.1–7.7 done. **Story 7.7 (prerender exclusions + dark mode regression) review patches complete** — route-registry-backed prerender guardrail, screenshot evidence, full required-route axe coverage with color contrast active, Epic 7 dark-token contrast audit, Lighthouse score summary artifact, and cross-model sign-off. Story 7.8 (new-route smoke tests + coverage floor) is next.
