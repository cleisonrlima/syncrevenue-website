...
- **Epic 7 in-progress:** Stories 7.1–7.6 done; **Story 7.7 (prerender exclusions + dark mode regression) in review** — commit `9be6aae` pushed. Prerender exclusion model (`INCLUDED_ROUTES=['/']`, `EXCLUDED_ROUTES=['/v2','/demo','/dashboard/*','/admin/*','/privacy','/404']`, `KNOWN_ROUTES` defensive warn) implemented in `scripts/prerender.tsx`. Regression sweep: zero regressions found on all existing routes in forced dark mode. Lighthouse desktop+mobile pass. Axe sweep clean. 101 files / 862 tests green. Story 7.8 (new-route smoke tests + coverage floor) is next.
