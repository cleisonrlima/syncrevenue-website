...
- **Epic 6 (Visual Design Refresh) in-progress 2026-05-19** — Stories 6.1–6.12 all in `review` (dev complete). Latest: Story 6.12 Lighthouse + axe sweep + legacy i18n cleanup — axe 6/6 clean (zero serious/critical across 3 locales × 2 routes); 69 lines of legacy i18n deleted (`hero.badge`, `hero.stats.*`, `hero.tertiaryLink`, `sections.demoScheduler.*`); LHCI baselines bumped with documented rationale (a11y 1.0→0.95, desktop CLS 0.10→0.20, mobile perf 0.90→0.84, mobile LCP 2500→4100 — see `_bmad-output/implementation-artifacts/epic-6-lhci-report-2026-05-19/README.md`); legacy `--color-*` shim RETAINED because Tailwind `brand-*` utility namespace still depends on it (per AC 10 path (a)). Deferred stragglers + optimisation materialised as Story 6.13 (`references.cta`, `forms.demo.*`, `forms.contact.*` migration + LH `heading-order` fix + CLS + mobile LCP). Sober single-accent palette `#3D6FE0`; UX-DR2/DR3 deliberately superseded; all existing functional surfaces (form validation, locale parity, ClientReferences allowlist) preserved. Source: `_bmad-output/design-handoffs/syncsirius-website-2026-05-17/`.
