...
- **Epic 7 in-progress:** Stories 7.1–7.5 are done. **Story 7.5 (i18n extraction) closed after senior review patches:** added 3 new namespaces (`landing.*`, `figmaDemo.*`, `dashboard.*`) to en/pt-BR/es with full tree-shape parity; wired `useTranslation` into 8 Epic 7 surfaces; added route-owned `LanguageSwitcher` controls to Landing, Demo, and DashboardLayout; localized chart tooltip/series labels; made translated statuses searchable; added neutral unknown-status handling, row-specific action labels, interpolation placeholder parity, and route-level locale tests. `npm run typecheck` green; `npm run test:run` green (101 files / 859 tests); `npm run build` green with existing Hero.tsx `fetchPriority` warning. AC 1 deviation documented: `/demo` strings landed under `figmaDemo.*` (not `demo.*`) due to namespace collision with Epic 1/2 DemoScheduler keys — approved by user 2026-05-22.
