...
- **Epic 7 in-progress:** Stories 7.1–7.7 done. **Story 7.8 (new-route smoke tests + coverage floor) → review.** 2 new Playwright journey specs (`v2-to-demo.spec.ts`, `dashboard-nav.spec.ts`), axe spec extended with 7 new Epic 7 route scans, Vitest coverage thresholds added to `vite.config.ts`. Baseline: 102 files / 868 tests green ×3. typecheck + build + test-build-output all exit 0. Bundle sizes documented; route-level code-split confirmed.
