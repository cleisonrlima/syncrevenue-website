...
- **Epic 7 in-progress:** Stories 7.1, 7.2, 7.3, and 7.4 are done. **Story 7.5 (i18n extraction) now `review`:** added 3 new namespaces (`landing.*`, `figmaDemo.*`, `dashboard.*`) to en/pt-BR/es with full tree-shape parity; wired `useTranslation` into 8 Epic 7 surfaces (Landing, Demo, DashboardLayout, DashboardHome, RevenueRecovery, Payouts, Insights, Settings); extended parity guard in `Sections.i18n.test.tsx` with `REQUIRED_EPIC_7_PATHS` (~200 leaf keys covering landing / figmaDemo / dashboard.{layout,status,overview,recovery,payouts,insights,settings}); `npm run typecheck` green; `npm run test:run` green (101 files / 854 tests); `npm run build` green. AC 1 deviation documented: `/demo` strings landed under `figmaDemo.*` (not `demo.*`) due to namespace collision with Epic 1/2 DemoScheduler keys — approved by user 2026-05-22. Awaiting cross-model review.
