...
|---|---|---|
| `src/components/layout/AdminLayout.tsx` | `/admin` shell | Auth gate + `/me` bootstrap + locale-stripped meta head; persistent nav (Dashboard / Leads / Team `NavLink`s + Logout) above `<Outlet />` when authenticated (Story 4.6) |
| `src/pages/admin/Login.tsx` | `/admin/login` | i18n login form, accessible error via `role="alert"` |
| `src/pages/admin/Dashboard.tsx` | `/admin/dashboard` | Stats grid (`getAdminDashboardStats`) — total / pending / new-this-week / leads-by-locale; Skeleton load state, `role="alert"` on error with Retry; 401 → `clearSession()` (Story 4.6) |
...
| `src/pages/admin/Team.tsx` | `/admin/team` | Team CRUD list + inline create/edit form (Story 4.4) — uses `getAdminTeam` / `postAdminTeam` / `putAdminTeam`; per-field blur validation via `src/lib/team-schema.ts`; includes localized `experience_*` footer meta fields from Story 6.7 review; 401 → `clearSession()`; 400 `field` → inline error on offending input; per-row active toggle with optimistic update + revert + per-row `role="alert"` (Story 4.5) |
...
| `src/lib/api.ts` | — | `postAdminLogin`, `postAdminLogout`, `getAdminMe`, `getAdminLeads`, `patchAdminLeadStatus` (Story 4.3), `getAdminTeam`/`postAdminTeam`/`putAdminTeam`/`getPublicTeam` (Story 4.4), `patchAdminTeamActive` (Story 4.5), `getAdminDashboardStats` + `parseAdminDashboardStats` + `AdminDashboardStats` type (Story 4.6), `AdminApiError`, `PublicTeamError` (Story 4.4 — public-surface failures never trip `clearSession`), `parseAdminLeadRow`, `parseAdminTeamMemberRow`, types `AdminLeadRow`/`AdminLeadStatus`/`AdminLeadLocale`/`AdminTeamMemberRow`/`AdminTeamMemberInput`/`PublicTeamMemberRow` |
...
| `server/routes/admin/team.ts` | GET `/api/admin/team`, POST `/api/admin/team`, PUT `/api/admin/team/:id` (Story 4.4); PATCH `/api/admin/team/:id/active` (Story 4.5) |
| `server/routes/admin/dashboard.ts` | GET `/api/admin/dashboard/stats` (Story 4.6) |
...
| `server/schemas/admin-team.schema.ts` | Zod `adminTeamCreateSchema` + `adminTeamUpdateSchema` (required name/roles/bios, URL-or-empty for `linkedin`/`photo_url`, coerced `order_index` ≥ 0, `active` silently stripped) + `adminTeamParamsSchema` (`id` coerced int > 0) — Story 4.4; `adminTeamActiveSchema` (`active` strict `0 \| 1`) — Story 4.5 |
...
| 4.5 | extended `server/schemas/admin-team.schema.ts` (`adminTeamActiveSchema`) + tests, extended `server/routes/admin/team.ts` (PATCH `/:id/active`) + tests, `src/lib/api.ts` `patchAdminTeamActive` + tests, extended `src/pages/admin/Team.tsx` (per-row toggle + optimistic update + revert + per-row alert) + tests, extended `tests/e2e/admin-team.spec.ts` (toggle → public exclude/include), i18n `admin.team.activeToggle` × 3 locales + `columns.toggle` |
| 4.6 | extended `server/dao/leads.dao.ts` (`countStats()` + `LeadsStats` interface, single-snapshot `database.transaction` over 4 prepared counts) + tests, new `server/routes/admin/dashboard.ts` (GET `/stats`) + new `dashboard.test.ts`, mounted `/api/admin/dashboard` in `server/index.ts` under `requireAdmin`, `src/lib/api.ts` `getAdminDashboardStats` + `parseAdminDashboardStats` + `AdminDashboardStats` type + tests, full rewrite of `src/pages/admin/Dashboard.tsx` (stats grid + Skeleton + role="alert" + retry; in-page logout removed) + new `Dashboard.test.tsx`, `src/components/layout/AdminLayout.tsx` persistent nav region (Dashboard/Leads/Team `NavLink`s + Logout) above `<Outlet />` + extended `AdminLayout.test.tsx`, new `tests/e2e/admin-dashboard.spec.ts` (unauth redirect + nav + logout + stats), updated `tests/e2e/admin-auth.spec.ts` to use `admin-nav-logout` testid, i18n `admin.nav.*` + `admin.dashboard.stats.*` + `admin.dashboard.errors.*` × 3 locales |
