...
| 5.11 | `server/dao/health.dao.test.ts` (NEW) — 3 unit tests for `healthDao`: (1) `ping()` returns `true` on open `:memory:` DB, (2) `ping()` throws when DB is closed (exercises the path `server/routes/health.ts` catches → 503), (3) default `healthDao` singleton exposes a `ping` method. Mirrors the canonical `:memory:` DAO test pattern (admin/contacts/leads/team/audit). Closes Epic 5 TEA finding G2 — every DAO in `server/dao/` now has dedicated test coverage. No production-code change. |
| 5.12 | `server/routes/admin/auth.test.ts` (UPDATE) — pattern shift: bcryptjs is mocked module-wide with a deterministic constant-time stub (no cost-factor work) so every test finishes in well under 100ms; previously every login/dummy-compare ran `bcrypt.compareSync` at cost 12 (~250-300ms) and stacked to >20s for the full file under CPU contention. The two assertions that previously synthesised "16 minutes ago" via `Date.now()` arithmetic now drive `vi.useFakeTimers()` + `vi.setSystemTime()` + `vi.advanceTimersByTime(16 * 60 * 1000)` instead of racing the wall clock. Per-test runtimes dropped from 700-2500ms to 13-21ms. No production code changed. See `_bmad-output/test-artifacts/test-design/test-design-epic-5-v2.md` (NG2) for full rationale. |
