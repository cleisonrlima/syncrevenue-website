...
| 7.7 | Prerender Exclusions + Site-Wide Dark Mode Regression Sweep | SYN-495 | `[r]` review — prerender exclusion model (INCLUDED_ROUTES/EXCLUDED_ROUTES/KNOWN_ROUTES + defensive console.warn) implemented in `scripts/prerender.tsx`. Regression sweep: zero regressions found across all existing routes in forced dark mode. Lighthouse desktop+mobile pass. Axe 0 critical/serious violations. 101 files / 862 tests green. commit `9be6aae` pushed. Jira sync pending. |
