...
**Jira:** [SYN-488](https://xillinha.atlassian.net/browse/SYN-488) (Epic, In Progress) + stories SYN-489..496 + 50 sub-tasks SYN-497..546 — 7.1–7.5 Done (SYN-489..493 + sub-tasks SYN-497..525); 7.6–7.8 To Do (SYN-494..496 + sub-tasks SYN-526..546). Reconciled 2026-05-22.
...
| 7.3 | Dashboard Pages — Overview, Revenue Recovery, Payouts, Insights, Settings | SYN-491 | `[x]` done — review patches fixed: discrepancy deltas, chart/search filters, empty states, disabled inert pagination, API key reveal, integration toggles. |
| 7.4 | Landing at `/v2` + DemoForm at `/demo` | SYN-492 | `[x]` done — review patches fixed: lazy fallback, keyed ErrorBoundary, App route body assertions, Demo landmark/mobile grid, Landing slick override, mobile menu a11y, footer links, scroll init. |
| 7.5 | i18n Extraction for Epic 7 Pages — en / pt-BR / es | SYN-493 | `[x]` done — 3 new namespaces (landing.*, figmaDemo.*, dashboard.*) added across en/pt-BR/es with full tree-shape parity; useTranslation wired into 8 Epic 7 surfaces; review patches added route-owned language switchers, localized chart tooltip/series labels, translated-status search, neutral unknown-status handling, row-specific action labels, interpolation placeholder parity, and route-level locale tests. typecheck / test:run (101 files / 859 tests) / build all green. AC 1 deviation: /demo strings landed under `figmaDemo.*` (not `demo.*`) due to namespace collision with Epic 1/2 DemoScheduler keys — approved by user 2026-05-22. |
| 7.6 | Brand Copy Rewrite — SyncSyrius/Insurance → SyncRevenue/Travel Commission | SYN-494 | `[r]` review — brand name sweep (SyncSyrius→SyncRevenue product / Sync Sirius company per decision matrix); domain vocab sweep (carrier→GDS, policy→PNR/ticket, clawback→debit memo/ADM, SyncPay→SyncRevenue Payouts, SyncInsights Enterprise→SyncRevenue Insights); GDS provider mock data (Global Life/Apex Health/Prime Auto/SecureCare → Amadeus/Sabre/Travelport with PNR IDs and travel client names); Settings defaults expanded (BRL/Sao_Paulo first; em-dash currency labels; newyork/london timezone keys added); SEO/OG copy rewritten across all 5 dashboard pages + landing/demo (replaced scaffold boilerplate with real product descriptions in en/pt-BR/es); 101 files / 859 tests green; typecheck green; build green; commit `9558233` pushed; Jira SYN-494 + 7 sub-tasks (SYN-526..532) synced to Done/Review. |
| 7.7 | Prerender Exclusions + Site-Wide Dark Mode Regression Sweep | SYN-495 | `[x]` done — code review patches complete: route-registry-backed prerender guardrail, screenshot evidence, full required-route axe coverage with color contrast active, Epic 7 dark-token contrast audit, Lighthouse score summary artifact, and cross-model sign-off. No follow-up stories required. |
| 7.8 | New-Route Smoke Tests + Vitest Coverage Floor | SYN-496 | `[x]` done — review patches complete: mobile Playwright journey fixed, axe spec extended with 7 new Epic 7 route scans, runnable Vitest coverage floor + CI gate added, build chunk warning removed, `/dashboard/insights` decorative chart a11y patched, and TEA baseline recorded in `_bmad-output/test-artifacts/story-7-8-tea-baseline.md`. Baseline: 102 files / 868 tests; coverage 88.69/78.94/91.27/91.40; typecheck/build/coverage/axe/mobile e2e green. |
