...
| 7.8 | New-Route Smoke Tests + Vitest Coverage Floor | SYN-496 | `[r]` review — 2 new Playwright specs (v2-to-demo, dashboard-nav), axe spec extended with 7 new Epic 7 route scans, Vitest coverage thresholds added to vite.config.ts. Baseline: 102 files / 868 tests green ×3. typecheck + build + test-build-output exit 0. Bundle sizes documented; code-split confirmed. |
